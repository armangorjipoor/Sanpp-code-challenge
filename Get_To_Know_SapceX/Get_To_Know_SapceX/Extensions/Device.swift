...
struct Device {
    ...
    // MARK: NEW IPHONES - X or NEWER
    
    static func IS_X_INCHES() -> Bool {
        return isPhone() && isSize(height: .Inched_X)
    }

    static func IS_X_INCHES_OR_LARGER() -> Bool {
        return isPhone() && isSizeOrLarger(height: .Inched_X)
    }

    static func IS_X_INCHES_OR_SMALLER() -> Bool {
        return isPhone() && isSizeOrSmaller(height: .Inched_X)
    }
